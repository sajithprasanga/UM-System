...
<?php 
	...
	if(isset($_POST['submit'])){
		...
		//checking max length
		$max_lan_fields = array('first_name' => 50 , 'last_name' => 100 , 'email' => 100 );
		...
		//checking if email address already 
		...
		if($result_set){
			if (mysqli_num_rows($result_set) == 1){
				$errors[] = '-Email Address allready exsist';
			}
		}
		...
		if(empty($errors)){ 
			...
			if($result){
				...
				$errors[] = '-Feild to modify the record ';
			}  
		}
 	...
 	<main>
 		...
 		<form action="Modify-user.php" method="post" class="userform">
 			...
 			<p>
 				...
 				<span>******** | <a href="change-password.php?user_id=<?php echo $user_id; ?>">Change Password</a></span>
 			</p>
